...
export default async function filterConfig(cwd: string, conf: Configuration): Promise<Configuration> {
  ...
  try {
    ...
    warn(`Failed to load packi/webpack.filter.js, message: ${(err as Error).message}`);
  ...
  try {
    ...
    warn(`Failed to run packi/webpack.filter.js, message: ${(err as Error).message}`);
    warn(`Fallback to default webpack configuration.`);
    return conf;
  }
}
