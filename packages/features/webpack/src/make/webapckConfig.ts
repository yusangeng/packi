...
function getOptimization(isDev: boolean, generateReport: boolean) {
  if (isDev) {
    return {};
  }
  ...
  return {
    ...
    concatenateModules: !generateReport,
    minimize: true,
    minimizer: [
...
function getDevtool(isDev: boolean) {
  return isDev ? "eval-cheap-module-source-map" : void 0;
}
...
export default function webpackConfig({
  ...
  config.optimization = getOptimization(isDev, generateReport);
  config.resolve = getResolve(cwd);
  config.resolveLoader = getResolveLoader(cwd);
  config.performance = getPerformance();

  addLoadersAndPlugins(config, { cwd, mode, target, generateReport, viewportWidth });
